#!/bin/bash
#Compiler FileZilla_3.60.2 with storj by KAPITALSIN 2022 B30072022
#sudo apt-get git subversion 
#https://tecadmin.net/install-go-on-ubuntu/
#sudo add-apt-repository ppa:ubuntu-toolchain-r/test
#
# B30072022
# Updated FileZilla Client to 3.60.2 and libfilezilla to 0.38.1
# Added more binaries in the bin folder
#
# B26062022
...
sudo apt install p7zip wget libjson-c-dev libuv1-dev libmicrohttpd-dev libgtk2.0-dev libcurl4-gnutls-dev libdbus-1-dev wx-common libtool git -y
...
wget https://download.filezilla-project.org/libfilezilla/libfilezilla-0.38.1.tar.bz2 -qO-|tar -C $SRC -xj
...
wget https://download.filezilla-project.org/client/FileZilla_3.60.2_src.tar.bz2 -qO-|tar -C $SRC -xj
pushd $SRC/filezilla*/
...
pushd $OUT
mkdir -p FileZilla_3.60.2.storj.x86_64/bin

strip bin/*
cp bin/* FileZilla_3.60.2.storj.x86_64/bin
rm FileZilla_3.60.2.storj.x86_64/bin/*debug*
cp -r share FileZilla_3.60.2.storj.x86_64

echo '#!/bin/sh' > $OUT/FileZilla_3.60.2.storj.x86_64/filezilla.sh
echo 'BIN=bin/./filezilla' >> $OUT/FileZilla_3.60.2.storj.x86_64/filezilla.sh
echo 'export LD_LIBRARY_PATH=lib/x86_64-linux-gnu:/lib64:usr/lib/x86_64-linux-gnu/' >> $OUT/FileZilla_3.60.2.storj.x86_64/filezilla.sh
echo 'exec $BIN $@' >> $OUT/FileZilla_3.60.2.storj.x86_64/filezilla.sh
chmod a+x $OUT/FileZilla_3.60.2.storj.x86_64/filezilla.sh

for library in $(ldd "bin/certtool" "bin/filezilla" "bin/fzputtygen"  "bin/fzsftp" | cut -d '>' -f 2 | awk '{print $1}')
do
	[ -f "${library}" ] && cp --verbose --parents "${library}" "$OUT/FileZilla_3.60.2.storj.x86_64"
done

cp $OUT/lib/libuplink.so FileZilla_3.60.2.storj.x86_64/lib/x86_64-linux-gnu/

mv $OUT/FileZilla_3.60.2.storj.x86_64 ../FileZilla_3.60.2.storj.x86_64_GLIBC_"$glibc"
popd
echo "FILEZILLA COMPILED AT $STAGING/FileZilla_3.60.2.storj.x86_64_GLIBC_"$glibc""
