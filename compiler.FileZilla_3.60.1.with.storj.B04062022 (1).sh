...
requiredver="9"
 if [ "$(printf '%s\n' "$requiredver" "$currentver" | sort -V | head -n1)" = "$requiredver" ]; then 
        ...
        echo "GCC version "$currentver" found, version ${requiredver} or greater is required to compile Filezilla, now we are going to install the required GCC"

notify-send "INSTALLING NEW GCC"
echo "INSTALLING NEW GCC"
sudo apt-get install software-properties-common && sudo add-apt-repository -y ppa:ubuntu-toolchain-r/test && sudo apt-get update && sudo apt-get install gcc-9 g++-9 -y && sudo update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-7 7 && sudo update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-9 9 && sudo update-alternatives --install /usr/bin/g++ g++ /usr/bin/g++-9 9 && sudo update-alternatives --auto gcc
fi
